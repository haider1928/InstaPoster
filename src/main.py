...
def save_headline(headline: str) -> None:
...
def process_news() -> tuple[str, bool]:
    ...
    headlines = NewsService.get_headlines()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate images from news headlines and Hadith, then optionally upload them to Instagram."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Generate content images without logging into Instagram or posting them.",
    )
...
def main(dry_run: bool = False) -> None:
    ...
    if dry_run:
        logger.info("Dry run enabled. Skipping Instagram login and upload.")
    else:
    ...
    if news_success:
        if dry_run:
            logger.info("News image generated for dry run: %s", Config.OUTPUT_NEWS_IMG)
        else:
    ...
    if hadith_success:
        if dry_run:
            logger.info("Hadith image generated for dry run: %s", Config.OUTPUT_HADITH_IMG)
        else:
